...
export interface TusAIApprovalRequirement { required: boolean; policy: string; requirementId?: string; workflowId?: string }
export interface TusAIAuditReference { auditIds: string[]; eventIds: string[]; digitalTwinRefs: string[]; approvalReference?: string }
...
export interface TusAIRecommendation extends TusEntityBase<'ai-recommendation'> { activity: string; targetRef: TusReference; summary: string; recommendationId: string; confidence: number; evidenceRefs: string[]; modelVersion: string; generatedAt: string; approvalRequirement: TusAIApprovalRequirement; auditReference: TusAIAuditReference; advisoryOnly: true; requestedAction?: string; approvalRefs: TusReference<'approval'>[]; modelLineageRefs: string[] }
...
export function validateTusEntity(entity: unknown): { valid: boolean; errors: string[] } {
  ...
  if (kind === 'ai-recommendation') {
    const recommendation = tusEntity as TusAIRecommendation;
    if (recommendation.recommendationId !== recommendation.id) errors.push('ai-recommendation.recommendationId must match id');
    if ((recommendation.evidenceRefs ?? []).length === 0) errors.push('ai-recommendation requires evidenceRefs');
