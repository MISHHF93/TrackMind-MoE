...
const requiredEventMetadata = ['eventType','version','timestamp','actor','correlationId','causationId','aggregateId','tenantId','racetrackId','payload','auditRef','digitalTwinRef'] as const;
...
export function createTrackMindNexusUpgradePackage(generatedAt = new Date().toISOString()): TrackMindNexusUpgradePackage {
  const workspaces: NexusWorkspaceUpgrade[] = [
    ...
    workspace({ id: 'track-configuration', title: 'Track Configuration', route: '/track-configuration', apiPath: '/api/v1/track-configuration/map', owner: 'Track Configuration', status: 'partial', approvalRequiredActions: ['rail position change','turf configuration change'], eventTypes: ['gate.movement.requested.v1','workflow.instance.transitioned.v1'], auditActions: ['track.configuration.changed'], twinKinds: ['racetrack','track-sector','starting-gate'], observabilityMetrics: ['configuration_change_count','gps_verification_status'], testCoverage: ['validation schemas','Digital Twin synchronization'] }),
    workspace({ id: 'assets', title: 'Asset Registry', route: '/assets', apiPath: '/api/v1/assets', owner: 'Asset Registry', status: 'implemented', approvalRequiredActions: ['critical asset lifecycle change'], eventTypes: ['asset.registry.changed.v1'], auditActions: ['asset.created','asset.updated','asset.approved'], twinKinds: ['racetrack','starting-gate','horse','barn','facility','camera','sensor','vehicle','emergency-asset','ai-agent'], observabilityMetrics: ['asset_count','critical_asset_count'], testCoverage: ['tenant isolation','event/audit/twin sync'] }),
...
export function buildNexusUpgradeEventEnvelope(input: { eventId: string; eventType: NexusEventContract['eventType']; tenantId: string; racetrackId: string; actorId: string; subjectId: string; subjectType: string; correlationId: string; auditRef: string; digitalTwinRef?: string; payload?: Record<string, unknown>; evidence?: string[]; occurredAt?: string }): NexusEventEnvelope {
  const event: NexusEventEnvelope = { eventId: input.eventId, eventType: input.eventType, tenantId: input.tenantId, occurredAt: input.occurredAt ?? new Date().toISOString(), actor: { id: input.actorId, type: 'service' }, correlationId: input.correlationId, subject: { id: input.subjectId, type: input.subjectType, tenantId: input.tenantId }, payload: { racetrackId: input.racetrackId, auditRef: input.auditRef, digitalTwinRef: input.digitalTwinRef, ...(input.payload ?? {}) }, evidence: input.evidence ?? [input.auditRef] };
