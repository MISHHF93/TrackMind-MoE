...
export type DomainEntityKind =
  ...
  | 'racetrack' | 'race-meet' | 'racing-season' | 'race-day' | 'race-card' | 'race-entry' | 'race'
  | 'horse' | 'jockey' | 'trainer' | 'owner' | 'veterinarian' | 'steward' | 'official'
  | 'barn' | 'stall' | 'track-sector' | 'facility' | 'asset' | 'starting-gate' | 'sensor' | 'vehicle'
  | 'fan' | 'data-provider' | 'federation-participant' | 'finance-record'
...
export interface RaceMeetEntity extends DomainEntityBase<'race-meet'> { racetrackId: EntityId; seasonId?: EntityId; meetCode?: string; season: string; opensOn: string; closesOn: string; status: 'scheduled' | 'open' | 'closed' | 'cancelled'; raceDayIds: EntityId[]; regulatoryAuthority?: string; calendarStatus?: 'planned' | 'scheduled' | 'approved' | 'active' | 'completed' | 'cancelled' }
export interface RacingSeasonEntity extends DomainEntityBase<'racing-season'> { racetrackId: EntityId; label: string; year: number; startsOn: string; endsOn: string; status: 'planned' | 'scheduled' | 'approved' | 'active' | 'completed' | 'cancelled'; meetIds: EntityId[]; regulatoryAuthority?: string }
export interface RaceDayEntity extends DomainEntityBase<'race-day'> { racetrackId: EntityId; raceMeetId?: EntityId; seasonId?: EntityId; raceDate: string; status: 'scheduled' | 'open' | 'closed' | 'cancelled'; calendarStatus?: 'planned' | 'scheduled' | 'approved' | 'active' | 'completed' | 'cancelled'; raceIds: EntityId[]; raceCardId?: EntityId }
export interface RaceCardEntity extends DomainEntityBase<'race-card'> { racetrackId: EntityId; raceMeetId?: EntityId; raceDayId: EntityId; raceDate: string; status: 'draft' | 'review' | 'approved' | 'published' | 'completed' | 'archived' | 'cancelled'; raceIds: EntityId[]; publishedAt?: ISODateTime; conditionBookVersion?: string; classification?: string; purseCents?: number }
export interface RaceEntryEntity extends DomainEntityBase<'race-entry'> { raceCardId: EntityId; raceId: EntityId; horseId: EntityId; trainerId: EntityId; jockeyId?: EntityId; ownerIds: EntityId[]; postPosition?: number; programNumber?: string; weightLbs?: number; status: 'entered' | 'declared' | 'scratched' | 'starter' | 'also-eligible' }
export interface RaceEntity extends DomainEntityBase<'race'> { racetrackId: EntityId; raceDayId: EntityId; raceMeetId?: EntityId; raceCardId?: EntityId; raceNumber: number; surface: 'dirt' | 'turf' | 'synthetic'; status: 'scheduled' | 'loading' | 'ready' | 'running' | 'stopped' | 'official' | 'cancelled'; entryHorseIds: EntityId[]; jockeyIds?: EntityId[]; stewardPanelId?: EntityId }
export interface HorseEntity extends DomainEntityBase<'horse'> { registrationNumber?: string; microchipId?: string; status: 'active' | 'scratched' | 'vet-flagged' | 'retired' | 'inactive'; lifecycleStatus?: 'active' | 'inactive' | 'retired' | 'deceased'; trainerId?: EntityId; ownerIds: EntityId[]; veterinarianIds?: EntityId[]; barnId?: EntityId; stallId?: EntityId; sireId?: EntityId; damId?: EntityId; studBook?: string; breedRegistry?: string; registrationRecordIds?: EntityId[]; clearanceStatus?: 'cleared' | 'pending' | 'restricted' | 'denied'; welfareBand?: 'excellent' | 'acceptable' | 'watch' | 'intervention-required' }
export interface JockeyEntity extends DomainEntityBase<'jockey'> { licenseNumber: string; status: 'active' | 'suspended' | 'inactive'; racetrackId?: EntityId; legalName?: string; licenseStatus?: 'active' | 'expired' | 'suspended' | 'pending-renewal'; licenseExpiresOn?: string; weightAllowanceLbs?: number; complianceStatus?: 'compliant' | 'under-review' | 'suspended' | 'finding-open'; eligibilityStatus?: 'eligible' | 'under-review' | 'suspended' | 'ineligible'; horseIds?: EntityId[]; raceIds?: EntityId[]; stewardInquiryIds?: EntityId[] }
export interface TrainerEntity extends DomainEntityBase<'trainer'> { licenseNumber: string; status: 'active' | 'suspended' | 'inactive'; racetrackId?: EntityId; legalName?: string; licenseStatus?: 'active' | 'expired' | 'suspended' | 'pending-renewal'; licenseExpiresOn?: string; complianceStatus?: 'compliant' | 'under-review' | 'suspended' | 'finding-open'; barnIds?: EntityId[]; horseIds?: EntityId[]; incidentIds?: EntityId[] }
...
export interface VeterinarianEntity extends DomainEntityBase<'veterinarian'> { licenseNumber: string; authorityScope: 'exam' | 'clearance' | 'regulatory'; status: 'active' | 'suspended' | 'inactive'; racetrackId?: EntityId; clearanceWorkflowIds?: EntityId[] }
...
export interface OfficialEntity extends DomainEntityBase<'official'> { licenseNumber?: string; officialRole: 'racing-secretary' | 'clerk-of-scales' | 'placing-judge' | 'paddock-judge' | 'identifier' | 'timer' | 'photo-finish' | 'starter' | 'other'; racetrackId: EntityId; status: 'active' | 'on-duty' | 'off-duty' | 'suspended' | 'inactive' }
export interface FanEntity extends DomainEntityBase<'fan'> { racetrackId: EntityId; ticketId?: EntityId; attendanceStatus: 'expected' | 'on-site' | 'departed' | 'no-show'; guestTier?: 'general' | 'club' | 'hospitality' | 'owner-box'; raceDayId?: EntityId }
export interface DataProviderEntity extends DomainEntityBase<'data-provider'> { providerCode: string; licenseId: string; feedTypes: string[]; status: 'registered' | 'active' | 'degraded' | 'suspended' | 'retired'; ingestionJobIds: EntityId[]; canonicalArtifactKinds: string[] }
export interface FederationParticipantEntity extends DomainEntityBase<'federation-participant'> { organizationId: EntityId; participantTrackIds: EntityId[]; dataSharingPolicyId: EntityId; cohortIds: EntityId[]; status: 'onboarding' | 'active' | 'suspended' | 'withdrawn'; governanceCouncilSeat: boolean }
export interface FinanceRecordEntity extends DomainEntityBase<'finance-record'> { racetrackId: EntityId; recordType: 'revenue' | 'expense' | 'budget' | 'payout' | 'reconciliation'; amountCents: number; currency: string; raceDayId?: EntityId; raceId?: EntityId; status: 'draft' | 'pending-approval' | 'approved' | 'posted' | 'reconciled' | 'void'; dualControlApproverIds: EntityId[] }
...
export type DomainEntity = OrganizationEntity | TenantEntity | UserEntity | RoleEntity | PermissionEntity | RacetrackEntity | RaceMeetEntity | RacingSeasonEntity | RaceDayEntity | RaceCardEntity | RaceEntryEntity | RaceEntity | HorseEntity | JockeyEntity | TrainerEntity | OwnerEntity | VeterinarianEntity | StewardEntity | OfficialEntity | FanEntity | DataProviderEntity | FederationParticipantEntity | FinanceRecordEntity | BarnEntity | StallEntity | TrackSectorEntity | FacilityEntity | AssetEntity | StartingGateEntity | SensorEntity | VehicleEntity | IncidentEntity | SecurityEventEntity | WorkflowEntity | AIRecommendationEntity | ApprovalEntity | AuditEventEntity | AuditRecordEntity | ComplianceRecordEntity;
...
export const domainSchemas = {
  ...
  'racing-season': schema('racing-season', [{ path: 'racetrackId', required: true, type: 'string' }, { path: 'label', required: true, type: 'string' }, { path: 'year', required: true, type: 'number', min: 2000 }, { path: 'startsOn', required: true, type: 'string' }, { path: 'endsOn', required: true, type: 'string' }, { path: 'status', required: true, type: 'string' }, { path: 'meetIds', required: true, type: 'array' }]),
  ...
  'race-card': schema('race-card', [{ path: 'racetrackId', required: true, type: 'string' }, { path: 'raceDayId', required: true, type: 'string' }, { path: 'raceDate', required: true, type: 'string' }, { path: 'status', required: true, type: 'string', values: ['draft','review','approved','published','completed','archived','cancelled'] }, { path: 'raceIds', required: true, type: 'array' }]),
  'race-entry': schema('race-entry', [{ path: 'raceCardId', required: true, type: 'string' }, { path: 'raceId', required: true, type: 'string' }, { path: 'horseId', required: true, type: 'string' }, { path: 'trainerId', required: true, type: 'string' }, { path: 'ownerIds', required: true, type: 'array' }, { path: 'status', required: true, type: 'string' }]),
  ...
  official: schema('official', [{ path: 'officialRole', required: true, type: 'string' }, { path: 'racetrackId', required: true, type: 'string' }, { path: 'status', required: true, type: 'string' }]),
  fan: schema('fan', [{ path: 'racetrackId', required: true, type: 'string' }, { path: 'attendanceStatus', required: true, type: 'string' }]),
  'data-provider': schema('data-provider', [{ path: 'providerCode', required: true, type: 'string' }, { path: 'licenseId', required: true, type: 'string' }, { path: 'feedTypes', required: true, type: 'array' }, { path: 'status', required: true, type: 'string' }, { path: 'ingestionJobIds', required: true, type: 'array' }, { path: 'canonicalArtifactKinds', required: true, type: 'array' }]),
  'federation-participant': schema('federation-participant', [{ path: 'organizationId', required: true, type: 'string' }, { path: 'participantTrackIds', required: true, type: 'array' }, { path: 'dataSharingPolicyId', required: true, type: 'string' }, { path: 'cohortIds', required: true, type: 'array' }, { path: 'status', required: true, type: 'string' }, { path: 'governanceCouncilSeat', required: true, type: 'boolean' }]),
  'finance-record': schema('finance-record', [{ path: 'racetrackId', required: true, type: 'string' }, { path: 'recordType', required: true, type: 'string' }, { path: 'amountCents', required: true, type: 'number' }, { path: 'currency', required: true, type: 'string' }, { path: 'status', required: true, type: 'string' }, { path: 'dualControlApproverIds', required: true, type: 'array' }]),
