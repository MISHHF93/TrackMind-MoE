...
export function createUnavailableWorkspace(route: RouteSupportMetadata, reason: string): WorkspaceViewModel {
  ...
  return {
    ...
    metrics: [
      textMetric('Workspace availability', 'Offline view', reason, 'warning', [{ label: 'View service status', path: '/admin', detail: 'Review backend availability and dependency status.' }]),
    ...
    panels: [
      {
        ...
        actions: [
          { label: 'View service status', path: '/admin', detail: 'Check backend and dependency status.' },
