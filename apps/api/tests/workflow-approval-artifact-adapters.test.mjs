...
test('workflow artifact builder adapts the canonical gate move workflow contract', () => {
  ...
  assert.deepEqual(approval.requiredApprovers, ['racing-secretary', 'track-superintendent']);
...
test('safety-critical gate move approval artifacts preserve workflow and approval requirements', () => {
  ...
  assert.deepEqual(workflowApproval.requiredApprovers, ['racing-secretary', 'track-superintendent']);
  ...
  assert.deepEqual(approvalArtifact.requiredApprovers, ['racing-secretary', 'track-superintendent']);
...
test('artifact adapters are server-authored snapshots without frontend local mutation semantics', () => {
  ...
  assert.deepEqual(definition.steps.find((step) => step.id === 'approve-gate-move').approvalRoles, ['racing-secretary', 'track-superintendent']);
  ...
  assert.deepEqual(buildApprovalArtifact(request).approvalSteps[0].requiredApprovers, ['racing-secretary']);
});
