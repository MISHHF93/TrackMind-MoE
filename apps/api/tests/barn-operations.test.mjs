...
test('barn incident links are audited and emitted without browser-side mutation', () => {
  ...
  assert.ok(snapshot.barns[0].incidentIds.includes('incident-security-2'));
  assert.ok(snapshot.incidentLinks.some((item) => item.incidentId === 'incident-security-2'));
  assert.equal(eventBus.events({ type:'barn.incident.linked', aggregateId:'barn-a' }).length, 1);
  assert.ok(auditLog.forensicTimeline({ subjectId:'barn-a' }).some((entry) => entry.payload.action === 'barn.incident.linked'));
});

test('restricted stall assignment requires approval policy token', () => { const { service, approvals } = seeded(); const request = approvals.createRequest({ id:'approval-stall-a2', tenantId:'tenant-1', racetrackId:'tenant-1', action:'safety-critical-control', target:'stall-a2', requestedBy:'super-1', actorType:'human', reason:'quarantine exception', evidence:['human-approval-record'] }); approvals.decide(request.id, superintendent, 'approved', 'ops approve', ['human-approval-record'], '2026-06-13T00:01:00.000Z'); approvals.decide(request.id, steward, 'approved', 'steward approve', ['human-approval-record'], '2026-06-13T00:02:00.000Z'); const token = approvals.authorizeExecution({ requestId:request.id, action:'safety-critical-control', target:'stall-a2', tenantId:'tenant-1', racetrackId:'tenant-1', actor:superintendent, now:'2026-06-13T00:03:00.000Z' }); service.createRestriction({ actor:admin, barnId:'barn-a', stallId:'stall-a2', type:'quarantine', reason:'monitor horse', at:'2026-06-13T00:03:00.000Z', approvalToken:token }); assert.throws(() => service.assignHorse({ actor:admin, horseId:'horse-9', barnId:'barn-a', stallId:'stall-a2', assignedAt:'2026-06-13T00:04:00.000Z', reason:'without token' }), /requires approval token/); const occ = service.assignHorse({ actor:admin, horseId:'horse-9', barnId:'barn-a', stallId:'stall-a2', assignedAt:'2026-06-13T00:04:00.000Z', reason:'approved exception', approvalToken:token }); assert.equal(occ.stallId, 'stall-a2'); });
...
test('restricted trainer and access assignments are approval and audit gated', () => {
  ...
  const pending = approvals.createRequest({ id:'approval-pending-barn-a', tenantId:'tenant-1', racetrackId:'tenant-1', action:'safety-critical-control', target:'barn-a', requestedBy:'super-1', actorType:'human', reason:'restricted barn assignment review', evidence:['human-approval-record'] });
  const request = approvals.createRequest({ id:'approval-barn-a', tenantId:'tenant-1', racetrackId:'tenant-1', action:'safety-critical-control', target:'barn-a', requestedBy:'super-1', actorType:'human', reason:'restricted barn assignment', evidence:['human-approval-record'] });
  ...
  const token = approvals.authorizeExecution({ requestId:request.id, action:'safety-critical-control', target:'barn-a', tenantId:'tenant-1', racetrackId:'tenant-1', actor:superintendent, now:'2026-06-13T00:03:00.000Z' });
