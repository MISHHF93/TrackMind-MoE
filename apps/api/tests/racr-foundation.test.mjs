...
test('RACR versions assets, audits changes, emits events, and returns defensive copies', () => {
  ...
  assert.deepEqual(registry.events().map((event) => event.eventType), [racrRegistryEventTypes.assetCreated, racrRegistryEventTypes.assetUpdated]);
  ...
  assert.ok(governance.some((entry) => entry.schemaRef === 'racr.asset.created.v1' && entry.digitalTwinSync && entry.tenantScoped));
