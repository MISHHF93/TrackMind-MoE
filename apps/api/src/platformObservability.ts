...
export class PlatformObservabilityService {
  ...
  recordApiLatency(serviceId: string, route: string, latencyMs: number, statusCode = 200): PlatformTelemetrySignal { this.latencies.push(latencyMs); return signal('metric', 'api.request.latency', serviceId, latencyMs > 500 || statusCode >= 500 ? 'warning' : 'info', { route, latencyMs, statusCode }); }
