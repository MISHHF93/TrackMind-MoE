...
export class RacetrackAssetRegistryService {
  ...
  private assertSafetyApproval(asset: RegistryAsset, principal: AssetPrincipal, options: AssetCommandOptions): void { if (!this.requiresSafetyApproval(asset)) return; this.approvalService.assertAuthorized(options.approvalToken, 'safety-critical-control', asset.assetId, asset.tenantId, racetrackRef(asset), new Date().toISOString()); if (options.actorType === 'ai-agent') throw new Error('AI agents cannot execute safety-critical asset changes'); if (!principal.id) throw new Error('authenticated human or service actor is required for safety-critical asset changes'); }
