...
export class FacilitiesMaintenanceService {
  ...
  private async patchTwin(asset: RegistryAsset, patch: Record<string, unknown>, actor: string, eventId: string): Promise<void> {
    ...
    await this.eventBus.publish({ id: `${eventId}:twin`, type: 'digital-twin.state.patch', aggregateId: asset.digitalTwin.twinId, producer: 'facilities-maintenance', payload: { twinId: asset.digitalTwin.twinId, patch, actor, observedAt: new Date().toISOString() }, metadata: { team: 'facilities-maintenance', accountableRole: 'facilities-supervisor', compliance: 'regulated' } });
  }
  private async publish(type: string, aggregateId: string, payload: Record<string, unknown>): Promise<RaceDayEvent> {
    return this.eventBus.publish({ type, aggregateId, producer: 'facilities-maintenance', payload, metadata: { team: 'facilities-maintenance', accountableRole: 'facilities-supervisor', compliance: 'regulated' } });
  }
  private registerEventSchemas(): void {
    ['facilities.asset.seeded', 'facilities.inspection.recorded', 'facilities.preventive-maintenance.scheduled', 'facilities.work-order.requested', 'facilities.work-order.completed', 'facilities.readiness.evaluated', 'facilities.predictive-maintenance.flagged'].forEach((type) => this.eventBus.registerEvent({ type, version: 1, description: `Facilities maintenance ${type}`, owner: { service: 'facilities-maintenance', team: 'facilities-maintenance', accountableRole: 'facilities-supervisor' }, payloadFields: ['assetId', 'tenantId', 'actor'], compliance: 'regulated' }));
  }
}
...
export function facilitiesMaintenanceWorkflow(tenantId: string): WorkflowDefinition {
  return {
    ...
    startStepId: 'triage',
    ownerRole: 'facilities-supervisor',
    tenantId,
    triggerEvents: ['facilities.work-order.requested'],
    steps: [
      { id: 'triage', name: 'Triage facility asset health', type: 'userTask', role: 'maintenance-dispatcher', sla: { minutes: 20, escalationRole: 'facilities-supervisor', severity: 'warning' }, digitalTwin: { refs: [], syncMode: 'read-write', statePatch: { maintenanceTriage: 'started' } }, next: ['approval'] },
      { id: 'approval', name: 'Approve operational maintenance impact', type: 'approvalTask', role: 'facilities-supervisor', approvalRoles: ['facilities-supervisor', 'operations-command'], requiredApprovals: 1, sla: { minutes: 45, escalationRole: 'operations-director', severity: 'critical' }, next: ['schedule'] },
      { id: 'schedule', name: 'Schedule crew and lockout window', type: 'userTask', role: 'maintenance-lead', sla: { minutes: 60, escalationRole: 'facilities-supervisor', severity: 'warning' }, digitalTwin: { refs: [], syncMode: 'read-write', statePatch: { maintenanceScheduled: true } }, next: ['verify'] },
      { id: 'verify', name: 'Verify repair and return to service', type: 'userTask', role: 'facilities-supervisor', sla: { minutes: 60, escalationRole: 'operations-director', severity: 'critical' }, digitalTwin: { refs: [], syncMode: 'read-write', statePatch: { returnToServiceVerification: 'pending' } }, next: ['closed'] },
      { id: 'closed', name: 'Facilities work order closed', type: 'endEvent' },
    ],
  };
}
...
export function createMockFacilitiesMaintenanceWorkspace(timestamp = new Date().toISOString()): FacilitiesMaintenanceWorkspace {
  ...
  const inspections: FacilityInspectionRecord[] = [{ id: 'inspection-hvac-1', assetId: 'GRANDSTAND_HVAC_01', inspectedBy: 'facilities-supervisor', inspectedAt: timestamp, checklist: ['filter differential', 'motor temperature', 'patron-area airflow'], findings: ['filter pressure elevated'], status: 'watch', score: 78, nextInspectionDueAt: '2026-06-15T12:00:00.000Z', eventId: 'facilities.inspection.recorded', auditId: 'audit-facility-hvac-1', twinId: 'twin:GRANDSTAND_HVAC_01' }];
  const workOrders: FacilityWorkOrder[] = [{ id: 'wo-hvac-filter', assetId: 'GRANDSTAND_HVAC_01', title: 'Replace grandstand HVAC filters', priority: 'high', status: 'approval-required', operationalImpact: 'operational-impact', requestedBy: 'facilities-supervisor', requestedAt: timestamp, scheduledFor: '2026-06-14T18:00:00.000Z', dueAt: '2026-06-14T20:00:00.000Z', tasks: ['lockout unit', 'replace filters', 'verify airflow', 'return to service'], evidence: ['telemetry:filterDeltaPressure=68', 'inspection-hvac-1'], approvalRequestId: 'approval-facility-hvac', workflowInstanceId: 'facilities-maintenance-work-order-1', eventId: 'facilities.work-order.requested', auditId: 'audit-wo-hvac', twinId: 'twin:GRANDSTAND_HVAC_01' }];
