...
export function facilitiesMaintenanceWorkflow(tenantId: string): WorkflowDefinition {
  return {
    ...
    steps: [
      { id: 'triage', name: 'Triage facility asset health metadata', type: 'userTask', role: 'maintenance-dispatcher', sla: { minutes: 20, escalationRole: 'facilities-supervisor', severity: 'warning' }, digitalTwin: { refs: [], syncMode: 'read', statePatch: { maintenanceTriage: 'started' } }, next: ['approval'] },
      ...
      { id: 'schedule', name: 'Record crew and lockout-window metadata', type: 'userTask', role: 'maintenance-lead', sla: { minutes: 60, escalationRole: 'facilities-supervisor', severity: 'warning' }, digitalTwin: { refs: [], syncMode: 'read', statePatch: { maintenanceScheduled: true } }, next: ['verify'] },
      { id: 'verify', name: 'Record repair verification metadata', type: 'userTask', role: 'facilities-supervisor', sla: { minutes: 60, escalationRole: 'operations-director', severity: 'critical' }, digitalTwin: { refs: [], syncMode: 'read', statePatch: { returnToServiceVerification: 'pending' } }, next: ['closed'] },
