...
export const complianceEventContracts: EventContract[] = [
  { type: 'compliance.control.created', version: 1, description: 'Compliance control was created or baselined.', owner: { service: 'compliance-control-library', team: 'governance-risk-compliance', accountableRole: 'compliance-officer' }, payloadFields: ['controlId','frameworkIds','status'], compliance: 'regulated' },
  { type: 'compliance.evidence.collected', version: 1, description: 'Audit evidence was collected and linked to a control.', owner: { service: 'compliance-control-library', team: 'governance-risk-compliance', accountableRole: 'compliance-officer' }, payloadFields: ['controlId','evidenceIds','auditRecordId'], compliance: 'regulated' },
  { type: 'compliance.assessment.completed', version: 1, description: 'A control assessment was completed with evidence and findings.', owner: { service: 'compliance-control-library', team: 'governance-risk-compliance', accountableRole: 'compliance-officer' }, payloadFields: ['controlId','assessmentId','rating'], compliance: 'regulated' },
  { type: 'compliance.finding.opened', version: 1, description: 'A compliance finding was opened for corrective action.', owner: { service: 'compliance-control-library', team: 'governance-risk-compliance', accountableRole: 'compliance-officer' }, payloadFields: ['findingId','controlId','severity'], compliance: 'regulated' },
  { type: 'compliance.corrective-action.created', version: 1, description: 'A corrective action was opened and tied to workflow/approval governance.', owner: { service: 'compliance-control-library', team: 'governance-risk-compliance', accountableRole: 'compliance-officer' }, payloadFields: ['correctiveActionId','findingId','ownerId','dueAt'], compliance: 'regulated' },
  { type: 'compliance.accreditation.readiness.updated', version: 1, description: 'Accreditation readiness changed for a framework/program.', owner: { service: 'compliance-control-library', team: 'governance-risk-compliance', accountableRole: 'compliance-officer' }, payloadFields: ['programId','frameworkIds','readinessScore'], compliance: 'regulated' },
];
...
export class ComplianceControlLibrary {
  ...
  createEvidencePackage(pkg: Omit<EvidencePackage,'evidenceId'|'tenantId'|'racetrackId'|'source'|'auditRefs'|'eventRefs'|'aiRecommendationRefs'|'frameworkIds'|'frameworkMappings'|'controlOwnerId'|'reviewCadence'|'hisaOperationalOversightCategories'|'accreditationReadiness'|'sealed'|'readiness'> & Partial<Pick<EvidencePackage,'evidenceId'|'tenantId'|'racetrackId'|'source'|'auditRefs'|'eventRefs'|'aiRecommendationRefs'|'frameworkIds'|'frameworkMappings'|'controlOwnerId'|'reviewCadence'|'hisaOperationalOversightCategories'|'accreditationReadiness'|'sealed'|'readiness'>>) {
    ...
    const full: EvidencePackage = {
      ...
      accreditationReadiness: pkg.accreditationReadiness ?? { status: pkg.readiness === 'audit-ready' ? 'ready-for-review' : 'internal-review', score: this.readiness(controlIds).score, readinessOnly: true, externalCertificationClaimed: false, notes: 'Readiness and evidence packaging only; no external certification is claimed.' },
  ...
  private recordReadinessEvent(type: ComplianceAuditEvent['type'], occurredAt: string, frameworkIds: ComplianceFrameworkId[], auditRecordId: string, controlId?: string, workflowInstanceId?: string, approvalRequestId?: string) { this.auditReadinessEvents.push({ id: `compliance-event-${this.auditReadinessEvents.length + 1}`, type, occurredAt, frameworkIds: [...frameworkIds], controlId, auditRecordId, workflowInstanceId, approvalRequestId }); }
  ...
  private async publish(type: ComplianceAuditEvent['type'], payload: Record<string, unknown>, control: ComplianceControl | undefined, audit: AuditLogEntry, occurredAt: string, workflowInstanceId?: string, approvalRequestId?: string) {
    const event = await this.deps.eventBus?.publish({ type, payload, occurredAt, producer: 'compliance-control-library', aggregateId: control?.id ?? audit.subjectId, correlationId: audit.correlationId ?? audit.id, metadata: { compliance: 'regulated', team: 'governance-risk-compliance', accountableRole: 'compliance-officer', auditRecordId: audit.id } });
    if (event && control) {
      control.eventIds.push(event.id);
      const readinessEvent = this.auditReadinessEvents.at(-1);
      if (readinessEvent) readinessEvent.eventId = event.id;
    }
...
export function accreditationReadinessWorkflow(tenantId: string): WorkflowDefinition {
  return { id: 'accreditation-readiness-review', name: 'Accreditation readiness review', domain: 'compliance', version: '1.0.0', bpmnProcessId: 'Process_AccreditationReadinessReview', startStepId: 'scope', ownerRole: 'compliance-officer', tenantId, triggerEvents: ['compliance.accreditation.readiness.updated'], steps: [
    { id: 'scope', name: 'Scope accreditor and commission requirements', type: 'userTask', role: 'compliance-officer', sla: { minutes: 1440, escalationRole: 'compliance-director', severity: 'warning' }, next: ['evidence-package'] },
...
export function seededComplianceLibrary(tenantId = 'track-1') {
  ...
  const controls: Array<Omit<ComplianceControl,'evidenceIds'|'assessmentIds'|'workflowInstanceIds'|'auditRecordIds'|'approvalRequestIds'|'eventIds'|'reviewCadence'|'hisaOperationalOversightCategories'> & { reviewCadence?: ReviewCadence; hisaOperationalOversightCategories?: HisaOperationalOversightCategory[]; evidence: { id: string; uri: string; description: string; content: unknown }; rating: AssessmentRating; notes: string }> = [
    ...
    { id: 'ctrl-software-quality', title: 'Command center software quality controls', description: 'Frontend safety states, role-aware navigation, live/mock adapter contracts, and tests demonstrate ISO 25010 quality attributes.', frameworkIds: standardizeControlFrameworks(['ISO-25010','SOC-2']), status: 'implemented', ownerId: 'owner-compliance', obligationIds: ['obl-quality-reliability'], digitalTwinRefs: ['workflow:platform-health'], evidence: { id: 'ev-dashboard-quality', uri: 'test://dashboard/dashboard.test.mjs', description: 'Dashboard test coverage for loading, empty, error, degraded, permission, and approval-gated states.', content: { test: 'dashboard' } }, rating: 'effective', notes: 'Dashboard and API tests cover quality, usability, reliability, and safety-state behavior.' },
