...
export function seedWorkforceOperations(deps: ConstructorParameters<typeof WorkforceOperationsService>[1] = {}, tenantId = 'track-1', now = '2026-06-13T20:30:00.000Z'): WorkforceOperationsService {
  ...
  const identities: GovernedIdentity[] = [
    { id: 'staff-gate-lead', tenantId, kind: 'user', displayName: 'Marcus Hale', roles: ['organization-admin'], attributes: { department: 'race-operations', credentialed: true, title: 'Gate Crew Lead' } },
    { id: 'staff-vet-tech', tenantId, kind: 'user', displayName: 'Dr. Elena Vasquez', roles: ['veterinarian'], attributes: { department: 'veterinary', credentialed: true, title: 'Track Veterinarian' } },
    { id: 'staff-emergency-liaison', tenantId, kind: 'user', displayName: 'James Okonkwo', roles: ['security-manager'], attributes: { department: 'emergency', credentialed: true, title: 'Emergency Liaison' } },
    { id: 'staff-facilities-watch', tenantId, kind: 'user', displayName: 'Sofia Mendez', roles: ['facilities-manager'], attributes: { department: 'facilities', credentialed: true, title: 'Facilities Watch' } },
    { id: 'staff-starter-official', tenantId, kind: 'user', displayName: 'Robert "Bob" Kessler', roles: ['starter-official'], attributes: { department: 'race-operations', credentialed: true, title: 'Starter' } },
  ];
  identities.forEach((identity, index) => service.registerEmployee({ identity, employeeNumber: `EMP-${index + 101}`, department: index === 1 ? 'veterinary' : index === 2 ? 'emergency' : index === 3 ? 'facilities' : 'race-operations', managerIdentityId: 'staff-gate-lead', employmentStatus: 'active', emergencyQualified: index === 2 || index === 1, homeZoneId: index === 2 ? 'zone-grandstand' : index === 4 ? 'backstretch' : 'backstretch', hiredAt: '2024-01-01T00:00:00.000Z' }, 'seed', now));
  service.scheduleShift({ id: 'shift-race-7', tenantId, label: 'Race 7 operations shift', startsAt: '2026-06-13T19:30:00.000Z', endsAt: '2026-06-13T22:30:00.000Z', zoneId: 'backstretch', status: 'active', raceId: 'race-7', requirements: [
    ...
    { role: 'starter-official', demand: 1, certificationKinds: ['starter-license'], emergencyCritical: true },
  ] }, 'seed', now);
  ...
  service.upsertCertification({ id: 'cert-starter', tenantId, identityId: 'staff-starter-official', kind: 'starter-license', status: 'active', issuedAt: '2025-06-01T00:00:00.000Z', expiresAt: '2027-06-01T00:00:00.000Z', evidence: ['cert:starter-license'], requiredForRoles: ['starter-official'] }, 'workforce-compliance', now);
  ...
  service.upsertTraining({ id: 'training-starter', tenantId, identityId: 'staff-starter-official', courseId: 'gate-start-protocol', title: 'Starting gate protocol certification', status: 'completed', completedAt: now, dueAt: now, requiredForRoles: ['starter-official'], evidence: ['training:gate-start-protocol'] }, 'workforce-training', now);
  ...
  service.assign({ id: 'assign-starter', tenantId, identityId: 'staff-starter-official', role: 'starter-official', shiftId: 'shift-race-7', zoneId: 'backstretch', status: 'checked-in', certificationKinds: ['starter-license'], emergencyCritical: true, raceId: 'race-7' }, 'workforce-coordinator', now);
  return service;
}
