...
export function CommandCenter({ data, roles, authenticated = true, tenantId = 'saratoga', path = '/operations', serviceState = 'online', paletteQuery = '', user = { name: 'Avery Chen', title: 'Race Day Commander', roles } }: { data: Awaited<ReturnType<typeof loadCommandCenter>>; roles: Role[]; authenticated?: boolean; tenantId?: string; path?: string; serviceState?: ServiceState; paletteQuery?: string; user?: UserProfile }) {
  ...
  return (
    <main className="nexus-shell">
      ...
      {data.mode === 'mock' && <aside role="note">Mock adapter active: approved mock/live adapter boundary is read-only; state-changing controls only create approval requests.</aside>}
      <section aria-label="Frontend route and safety audit">
      ...
      <section aria-label="Platform Health workspace">
        ...
        <section aria-label="Service health and dependencies"><h3>Service health</h3>{data.platformHealth.services.map((service) => <article key={service.serviceId} data-status={service.status}><strong>{service.serviceId}: {service.status}</strong><p>Latency {service.latencyMs}ms; dependencies {service.dependencies.map((dep) => `${dep.id}:${dep.status}${dep.required ? ':required' : ''}`).join(', ') || 'none'}; checked {service.lastCheckedAt}</p></article>)}</section>
      ...
      <section aria-label="AI Governance workspace">
        ...
        <section aria-label="Active AI agents"><h3>Active agents</h3>{data.aiGovernance.activeAgents.map((agent) => <article key={agent.id}><strong>{agent.name}</strong><p>{agent.status}; model {agent.modelVersionId}; prompt {agent.promptTemplateId}; restricted {agent.restrictedActions.join(', ')}</p></article>)}</section>
        <section aria-label="AI recommendation queue"><h3>Recommendation queue</h3>{data.aiGovernance.recommendationQueue.map((rec) => <article key={rec.id}><RiskBadge level={rec.riskLevel} /><strong>{rec.recommendation}</strong><p>Confidence {Math.round(rec.confidence * 100)}%; affected assets {rec.affectedAssets.join(', ')}; approval policy {rec.approvalPolicy}; status {rec.status}</p><p>Evidence {rec.evidence.join(', ')}; lineage {rec.lineage.join(' → ')}</p></article>)}</section>
        <section aria-label="Safety-blocked AI actions"><h3>Safety-blocked actions</h3>{data.aiGovernance.safetyBlockedActions.map((blocked) => <article key={blocked.id} role="alert"><strong>{blocked.action}: {blocked.target}</strong><p>{blocked.reason}; approval policy {blocked.approvalPolicy}; confidence {Math.round(blocked.confidence * 100)}%</p><p>Evidence {blocked.evidence.join(', ')}; affected assets {blocked.affectedAssets.join(', ')}</p></article>)}</section>
        <section aria-label="AI evaluation status"><h3>Evaluation status</h3>{data.aiGovernance.evaluationStatus.map((item) => <article key={item.modelVersionId}><strong>{item.modelVersionId}: {item.status}</strong><p>Deployable {String(item.readiness.deployable)}; gaps {(item.readiness.gaps ?? []).join(', ') || 'none'}; explainability {item.latestEvaluation?.explainabilityScore}</p></article>)}</section>
        <section aria-label="AI evidence packages"><h3>Evidence packages</h3>{data.aiGovernance.evidencePackages.map((pkg) => <article key={pkg.id}><code>{pkg.hash}</code><p>{pkg.recommendationId}; evidence {pkg.evidence.join(', ')}; lineage {pkg.lineage.join(' → ')}</p></article>)}</section>
        <section aria-label="AI approval requirements"><h3>Approval requirements</h3>{data.aiGovernance.approvalRequirements.map((req) => <article key={req.id}><ApprovalChip status="pending-approval" /><strong>{req.policy}</strong><p>Roles {req.requiredRoles.join(', ')}; status {req.status}; evidence {req.evidence.join(', ')}</p></article>)}</section>
        <section aria-label="AI overrides and rollback records"><h3>Overrides and rollbacks</h3>{data.aiGovernance.overrides.map((o) => <p key={o.id}>Override {o.recommendationId}: {o.reason}; evidence {o.evidence.join(', ')}</p>)}{data.aiGovernance.rollbackRecords.map((r) => <p key={r.id}>Rollback {r.recommendationId} to {r.restoredVersionId}: {r.reason}; evidence {r.evidence.join(', ')}</p>)}</section>
        <section aria-label="AI monitoring metrics"><h3>Monitoring metrics</h3>{data.aiGovernance.monitoringMetrics.map((m) => <article key={`${m.modelId}-${m.metric}`}><strong>{m.metric}</strong><p>{m.modelId}; value {m.value}; threshold {m.threshold}; evidence {m.evidence.join(', ')}</p></article>)}</section>
        <section aria-label="AI audit trails"><h3>Audit trails</h3>{data.aiGovernance.auditTrails.map((audit) => <article key={audit.id}><code>{audit.action}</code><p>{audit.actor}; subject {audit.subject}; evidence {audit.evidence.join(', ')}</p></article>)}</section>
      </section>
      ...
      <section aria-label="Compliance Control Library dashboard">
        ...
        <section aria-label="Compliance framework placeholders"><h3>Framework placeholders</h3>{data.complianceLibrary.frameworks.map((framework) => <article key={framework.id}><strong>{framework.id}</strong><p>{framework.name}; placeholder: {String(framework.placeholder)}</p></article>)}</section>
        <section aria-label="Compliance controls"><h3>Controls</h3>{data.complianceLibrary.controls.map((control) => <article key={control.id}><strong>{control.title}</strong><p>{control.status}; owner {control.ownerId}; frameworks {control.frameworkIds.join(', ')}</p><p>Evidence {control.evidenceIds.join(', ')}; audit records {control.auditRecordIds.join(', ')}; workflows {control.workflowInstanceIds.join(', ')}</p></article>)}</section>
        <section aria-label="Compliance obligations"><h3>Obligations</h3>{data.complianceLibrary.obligations.map((obligation) => <p key={obligation.id}>{obligation.frameworkId}: {obligation.citation} — {obligation.summary}</p>)}</section>
        <section aria-label="Control owners and permissions"><h3>Control owners</h3>{data.complianceLibrary.owners.map((owner) => <p key={owner.id}>{owner.displayName}: {owner.role}; permissions {owner.permissions.join(', ')}</p>)}</section>
        <section aria-label="Compliance findings and corrective actions"><h3>Findings and corrective actions</h3>{data.complianceLibrary.findings.map((finding) => <article key={finding.id}><strong>{finding.severity}: {finding.summary}</strong><p>Status {finding.status}; actions {finding.correctiveActionIds.join(', ')}</p></article>)}{data.complianceLibrary.correctiveActions.map((action) => <p key={action.id}>{action.action}; due {action.dueAt}; workflow {action.workflowInstanceId}</p>)}</section>
        <section aria-label="Compliance review cycles"><h3>Review cycles</h3>{data.complianceLibrary.reviewCycles.map((cycle) => <p key={cycle.id}>{cycle.frameworkId} {cycle.periodStart}–{cycle.periodEnd}: {cycle.status}; score {cycle.readinessScore}</p>)}</section>
        <section aria-label="Audit readiness score by framework"><h3>Audit readiness by framework</h3>{data.complianceLibrary.readiness.byFramework.map((item) => <p key={item.frameworkId}>{item.frameworkId}: {item.score}% across {item.controls} controls</p>)}</section>
      </section>
      ...
      <section aria-label="Race Office workspace">
        ...
        <section aria-label="Race meets">{data.raceOffice.meets.map((meet) => <article key={meet.id}><h3>{meet.name ?? meet.id}</h3><p>{meet.trackId} · {meet.status}</p></article>)}</section>
        <section aria-label="Race days">{data.raceOffice.raceDays.map((day) => <article key={day.id}><h3>{day.raceDate}</h3><p>{day.status}; races {(day.raceIds ?? []).join(', ')}</p></article>)}</section>
        <section aria-label="Race cards">{data.raceOffice.cards.map((card) => <article key={card.id}><h3>Race {card.raceNumber}</h3><p>Status {card.status}; entries {card.entries}; scratches {card.scratches}; post positions {String(card.postPositionsDrawn)}.</p></article>)}</section>
      ...
      <section aria-label="Surface Intelligence workspace">
        ...
        <div aria-label="Surface status cards">{data.surfaceIntelligence.statusCards.map((card) => <StatusCard key={card.label} title={card.label} status={card.value} detail={`${card.detail} Tone: ${card.tone}`} />)}</div>
        <section aria-label="Surface sector table"><h3>Track sectors</h3><table><thead><tr><th>Sector</th><th>Surface</th><th>Score</th><th>Moisture</th><th>Compaction</th><th>Cushion</th><th>Drainage</th></tr></thead><tbody>{data.surfaceIntelligence.sectors.map((sector) => <tr key={sector.id}><td>{sector.name}</td><td>{sector.surfaceType}</td><td>{sector.conditionScore}</td><td>{sector.moisture}%</td><td>{sector.compaction}</td><td>{sector.cushionDepth}</td><td>{sector.drainageRate}</td></tr>)}</tbody></table></section>
        <section aria-label="Surface measurement timeline"><h3>Measurement timeline</h3><EventTimeline events={data.surfaceIntelligence.timeline.map((point) => ({ time: point.observedAt, label: `${point.kind}: ${point.label} ${point.value}; event ${point.eventId}; audit ${point.auditId}`, tone: point.kind }))} /></section>
        <section aria-label="Surface risk badges"><h3>Risk badges</h3>{data.surfaceIntelligence.riskBadges.map((risk) => <article key={risk.sectorId}><RiskBadge level={risk.level === 'moderate' ? 'medium' : risk.level} /><p>{risk.sectorId}: {risk.drivers.join(', ')}</p></article>)}</section>
        <section aria-label="Surface recommendation panel"><h3>Recommendations requiring approval</h3>{data.surfaceIntelligence.recommendations.map((item) => <article key={item.id}><strong>{item.recommendation}</strong><p>{item.type}; priority {item.priority}; approval required {String(item.requiresHumanApproval)}; execution {item.executionState}</p><p>Event {item.eventId}; audit {item.auditId}</p></article>)}</section>
        <section aria-label="Mock-safe surface map overlay"><h3>Heatmap-ready overlay</h3><p>{data.surfaceIntelligence.mock ? 'MOCK SAFE MAP OVERLAY - no operational surface state is mutated.' : 'Live read-only overlay; actions still require approval.'}</p>{data.surfaceIntelligence.heatmap.map((cell) => <article key={cell.id} data-latitude={cell.latitude} data-longitude={cell.longitude}><strong>{cell.sectorId}</strong><p>Moisture {cell.averageMoisture}; compaction {cell.averageCompaction}; drainage {cell.averageDrainage}; risk index {cell.riskIndex}</p></article>)}</section>
        <section aria-label="Surface Digital Twin sync"><h3>Digital Twin updates</h3>{data.surfaceIntelligence.digitalTwinSync.map((sync) => <article key={sync.twinId}><strong>{sync.twinId}</strong><p>{sync.status}; event {sync.eventId}; audit {sync.auditId}</p></article>)}</section>
      ...
      <section aria-label="Equine Intelligence workspace">
        ...
        <section aria-label="Horse ownership"><h3>Ownership</h3>{data.equineIntelligence.ownership.map((owner) => <article key={owner.ownerId}><strong>{owner.ownerName}</strong><p>{owner.percentage}% from {owner.effectiveFrom}</p></article>)}</section>
        <section aria-label="Trainer assignment"><h3>Trainer</h3>{data.equineIntelligence.trainerAssignments.map((trainer) => <article key={trainer.trainerId}><strong>{trainer.trainerName}</strong><p>License {trainer.licenseStatus}; effective {trainer.effectiveFrom}</p></article>)}</section>
        <section aria-label="Race history"><h3>Race history</h3>{data.equineIntelligence.raceHistory.map((race) => <article key={race.raceId}><strong>{race.raceId}</strong><p>{race.date}; {race.trackId}; {race.status}; finish {race.finishPosition ?? 'n/a'}</p></article>)}</section>
        <section aria-label="Workout history"><h3>Workout history</h3>{data.equineIntelligence.workoutHistory.map((workout) => <article key={workout.workoutId}><strong>{workout.distanceFurlongs}f in {workout.timeSeconds}s</strong><p>{workout.date}; {workout.surface}</p></article>)}</section>
        ...
        <section aria-label="Equine advisory AI recommendations"><h3>Advisory AI risk recommendations</h3>{data.equineIntelligence.aiRiskRecommendations.map((rec) => <article key={rec.id}><strong>{rec.summary}</strong><p>Advisory only {String(rec.advisoryOnly)}; veterinarian review required {String(rec.veterinarianReviewRequired)}; status {rec.status}</p></article>)}</section>
        <section aria-label="Equine approvals"><h3>Approvals</h3>{data.equineIntelligence.approvals.map((approval) => <article key={approval.id}><strong>{approval.action}</strong><p>{approval.status}; required role {approval.requiredRole}</p></article>)}</section>
      ...
      <section aria-label="Steward Center workspace">
        ...
        <section aria-label="Steward inquiry queue"><h3>Inquiry queue</h3>{data.stewardCenter.inquiries.map((inq) => <article key={inq.id}><strong>{inq.raceId}: {inq.status}</strong><p>Opened {inq.openedAt}; objections {inq.objections.length}; incidents {inq.incidentsUnderReview.length}</p></article>)}</section>
        {data.stewardCenter.inquiries.map((inq) => <section key={`${inq.id}-detail`} aria-label="Steward case detail"><h3>Case detail {inq.id}</h3><p>Horses: {inq.involvedHorses.map((h) => `${h.programNumber} ${h.name} locked=${h.officialResultLocked}`).join('; ')}</p><p>Jockeys: {inq.involvedJockeys.map((j) => `${j.name} (${j.licenseId})`).join('; ')}</p><p>Objections: {inq.objections.map((o) => `${o.allegation} [${o.status}]`).join('; ')}</p></section>)}
        <section aria-label="Steward evidence timeline"><h3>Evidence timeline</h3>{data.stewardCenter.inquiries.flatMap((inq) => inq.evidenceReferences).map((ev) => <article key={ev.id}><strong>{ev.kind}: {ev.description}</strong><p>{ev.capturedAt}; {ev.uri}; hash {ev.hash}; AI generated {String(Boolean(ev.aiGenerated))}</p></article>)}</section>
        <section aria-label="Steward rule reference panel"><h3>Rule reference panel</h3>{data.stewardCenter.inquiries.flatMap((inq) => inq.ruleReferences).map((rule) => <article key={rule.id}><strong>{rule.jurisdiction} {rule.rulebook} §{rule.section}</strong><p>{rule.citation}: {rule.summary}</p></article>)}</section>
        <section aria-label="Steward decision draft workflow"><h3>Decision draft workflow</h3>{data.stewardCenter.inquiries.flatMap((inq) => inq.decisionDrafts).map((draft) => <article key={draft.id}><strong>{draft.recommendation}</strong><p>Author {draft.authorRole}; AI {String(draft.aiGenerated)}; official ruling {String(draft.officialRuling)}</p><p>{draft.rationale}</p></article>)}</section>
        ...
        <section aria-label="Steward audit records"><h3>Audit records</h3>{data.stewardCenter.inquiries.flatMap((inq) => inq.auditRecords).map((audit) => <article key={audit.id}><code>{audit.hash}</code><p>{audit.actorId}; {audit.action}; subject {audit.subjectId}; previous {audit.previousHash}</p></article>)}</section>
      </section>
      ...
      <section aria-label="Security Operations workspace">
        ...
        <section aria-label="Restricted zones"><h3>Restricted zones</h3>{data.securityOperations.restrictedZones.map((zone) => <article key={zone.id}><strong>{zone.name}</strong><p>{zone.classification}; credential {zone.requiredCredential}; cameras {zone.cameraIds.join(', ')}</p></article>)}</section>
        <section aria-label="Access-control events"><h3>Access-control events</h3>{data.securityOperations.accessEvents.map((event) => <article key={event.id}><strong>{event.personDisplayName}: {event.decision}</strong><p>{event.zoneId}; credential {event.credentialId}; reason {event.reason}; event {event.eventId}; audit {event.auditId}</p></article>)}</section>
        <section aria-label="Camera assets"><h3>Camera assets</h3>{data.securityOperations.cameras.map((camera) => <article key={camera.id}><AssetHealthIndicator label={camera.label} status={camera.health === 'online' ? 'healthy' : camera.health === 'degraded' ? 'degraded' : 'offline'} /><strong>{camera.label}</strong><p>{camera.zoneId}; privacy masking {String(camera.privacyMasking)}; last heartbeat {camera.lastHeartbeatAt}</p></article>)}</section>
        <section aria-label="Security incidents"><h3>Security incidents</h3>{data.securityOperations.incidents.map((incident) => <article key={incident.id}><RiskBadge level={incident.severity} /><strong>{incident.title}</strong><p>{incident.status}; zone {incident.zoneId}; events {incident.eventIds.join(', ')}; audit {incident.auditId}</p></article>)}</section>
        ...
        <section aria-label="Investigation queue widget"><h3>Investigation queue</h3>{data.securityOperations.investigations.map((item) => <article key={item.id}><strong>{item.incidentId}: {item.status}</strong><p>Lead {item.lead}; evidence {item.evidence.join(', ')}; audit {item.auditId}</p></article>)}</section>
        <section aria-label="Watchlist placeholders"><h3>Watchlist placeholders</h3>{data.securityOperations.watchlistPlaceholders.map((item) => <article key={item.id}><strong>{item.displayLabel}</strong><p>{item.category}; notes {item.sensitiveNotes}; human review required {String(item.requiresHumanReview)}</p></article>)}</section>
        <section aria-label="Visitor logs"><h3>Visitor logs</h3>{data.securityOperations.visitorLogs.map((visitor) => <article key={visitor.id}><strong>{visitor.visitorDisplayName}</strong><p>Host {visitor.host}; zone {visitor.zoneId}; credential check {visitor.credentialCheckId}; audit {visitor.auditId}</p></article>)}</section>
        <section aria-label="Credential checks"><h3>Credential checks</h3>{data.securityOperations.credentialChecks.map((check) => <article key={check.id}><strong>{check.holderDisplayName}: {check.status}</strong><p>Decision {check.decision}; credential {check.credentialId}; audit {check.auditId}</p></article>)}</section>
        <section aria-label="Escalation workflows"><h3>Escalation workflows</h3>{data.securityOperations.escalations.map((flow) => <article key={flow.id}><strong>{flow.status}: {flow.reason}</strong><p>Route {flow.routeTo.join(' → ')}; audit {flow.auditId}</p></article>)}</section>
      ...
      <section aria-label="Emergency Operations command view">
        ...
        <section aria-label="Emergency plans"><h3>Emergency plans</h3>{data.emergencyOperations.plans.map((plan) => <article key={plan.id}><strong>{plan.name}</strong><p>Scenarios {plan.scenarios.join(', ')}; criteria {plan.activationCriteria.join(', ')}; drill cadence {plan.drillCadenceDays} days.</p></article>)}</section>
        <section aria-label="Incident command roles"><h3>Incident command roles</h3>{data.emergencyOperations.commandRoles.map((role) => <article key={role.id}><strong>{role.role}: {role.assignee}</strong><p>Permissions {role.permissions.join(', ')}</p></article>)}</section>
        <section aria-label="Emergency resource map"><h3>Resource map</h3>{data.emergencyOperations.resourceMap.map((resource) => <article key={resource.id} data-status={resource.status}><strong>{resource.label}</strong><p>{resource.kind}; {resource.zoneId}; {resource.coordinates.latitude}, {resource.coordinates.longitude}</p></article>)}</section>
        ...
        <section aria-label="Evacuation zones"><h3>Evacuation zones</h3>{data.emergencyOperations.evacuationZones.map((zone) => <article key={zone.id}><strong>{zone.name}: {zone.status}</strong><p>Route {zone.route.join(' → ')}; assembly {zone.assemblyArea}; capacity {zone.capacity}</p></article>)}</section>
        <section aria-label="Checklist progress"><h3>Checklist progress</h3><p>{data.emergencyOperations.checklist.filter((item) => item.completed).length} of {data.emergencyOperations.checklist.length} complete.</p>{data.emergencyOperations.checklist.map((item) => <label key={item.id}><input type="checkbox" checked={item.completed} readOnly /> {item.label} — human override {String(item.humanOverrideAvailable)}; AI blocking {String(item.aiBlockingAllowed)}</label>)}</section>
        <section aria-label="Communication log"><h3>Communication log</h3>{data.emergencyOperations.communicationLog.map((item) => <article key={item.id}><strong>{item.channel} to {item.audience}</strong><p>{item.message}; completed {String(item.completed)} {item.completedBy ? `by ${item.completedBy}` : ''}</p></article>)}</section>
        <section aria-label="Drills and after-action reports"><h3>Drills and after-action reports</h3>{data.emergencyOperations.drills.map((drill) => <article key={drill.id}><strong>{drill.scenario}</strong><p>Participants {drill.participants.join(', ')}; criteria {drill.successCriteria.join(', ')}</p></article>)}{data.emergencyOperations.afterActionReports.map((report) => <article key={report.incidentId}><strong>After-action {report.incidentId}</strong><p>Findings {report.findings.map((finding) => `${finding.finding} (${finding.owner})`).join('; ')}</p><p>Corrective actions {report.correctiveActions.map((action) => `${action.action} due ${action.dueDays}d`).join('; ')}</p></article>)}</section>
