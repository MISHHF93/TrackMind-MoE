...
import { navItems, type NavItem } from './navigation.js';

export interface TenantOption { id: string; name: string; timezone: string; status: 'open' | 'scheduled' | 'degraded' | 'emergency' }
export interface UserProfile { name: string; title: string; roles: Role[] }
export type ServiceState = 'online' | 'offline' | 'degraded';

export interface CommandPaletteItem { id: string; label: string; path: string; keywords: string[]; required?: Permission[] }

export const tenants: TenantOption[] = [
  { id: 'saratoga', name: 'Saratoga Race Course', timezone: 'America/New_York', status: 'open' },
  { id: 'belmont', name: 'Belmont Park', timezone: 'America/New_York', status: 'scheduled' },
  { id: 'mock-fallback', name: 'Mock Training Track', timezone: 'UTC', status: 'degraded' },
];
...
export function breadcrumbForPath(path: string, items: NavItem[] = navItems): string[] {
  const match = items.find((item) => item.path === path || path.startsWith(`${item.path}/`));
  return ['Nexus', match?.label ?? 'Operations Command'];
}
...
export function commandPaletteItems(roles: Role[]): CommandPaletteItem[] {
  return navItems
    ...
    .map((item) => ({ id: `go-${item.id}`, label: `Go to ${item.label}`, path: item.path, keywords: [item.id, item.label.toLowerCase(), item.path], required: item.required }));
}
